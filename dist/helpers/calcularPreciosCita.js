...
exports.resolverFilasDeLaCita = exports.calcularFilasDeLaCita = exports.normalizarCantidad = void 0;
...
exports.normalizarCantidad = normalizarCantidad;
/**
 ...
 */
const calcularFilasDeLaCita = (servicesData, descuento, transaction) => __awaiter(void 0, void 0, void 0, function* () {
    const ids = servicesData.map(fila => Number(fila.service_id));
    const servicios = yield service_1.default.findAll({ where: { id: ids }, transaction });
    ...
    const filas = servicesData.map(fila => {
        ...
        const pedida = servicio.por_unidad ? (0, exports.normalizarCantidad)(fila.cantidad) : 1;
         ...
         */
        const acompanado = servicios.some(otro => otro.id !== servicio.id && !otro.es_complemento);
        ...
        const unitario = acompanado && servicio.precio_agregado !== null
    ...
    const precioCita = filas.reduce((suma, fila) => suma + fila.appointment_service_price, 0);
    return { filas, precioCita };
});
exports.calcularFilasDeLaCita = calcularFilasDeLaCita;
/** Las filas tal como las guarda hoy el controlador, sin tocar nada. */
const filasDelNavegador = (servicesData) => servicesData.map(fila => ({
    service_id: fila.service_id,
    state: fila.state,
    appointment_service_price: fila.price,
    cantidad: (0, exports.normalizarCantidad)(fila.cantidad),
}));
/**
 ...
 */
const registrarDiferencias = (idCita, navegador, catalogo) => {
    const totalNavegador = navegador.reduce((suma, fila) => suma + Math.round(Number(fila.appointment_service_price) || 0), 0);
    const totalCatalogo = catalogo.reduce((suma, fila) => suma + fila.appointment_service_price, 0);
    const filasDistintas = navegador
        .map((fila, i) => ({ fila, esperada: catalogo[i] }))
        .filter(({ fila, esperada }) => esperada !== undefined &&
        (Math.round(Number(fila.appointment_service_price) || 0) !==
            esperada.appointment_service_price ||
            fila.cantidad !== esperada.cantidad));
    if (totalNavegador === totalCatalogo && filasDistintas.length === 0)
        return;
    console.warn(`[precio-sombra] cita ${idCita}: se guarda ${totalNavegador} y el catalogo da ${totalCatalogo}`);
    filasDistintas.forEach(({ fila, esperada }) => {
        console.warn(`[precio-sombra]   servicio ${fila.service_id}: se guarda ${fila.appointment_service_price} x${fila.cantidad}, el catalogo da ${esperada.appointment_service_price} x${esperada.cantidad}`);
    });
};
/**
 ...
 */
const resolverFilasDeLaCita = (_a) => __awaiter(void 0, [_a], void 0, function* ({ servicesData, descuento, precioDelNavegador, idCita, transaction, }) {
    if (RECALCULAR_PRECIOS_EN_SERVIDOR) {
        return (0, exports.calcularFilasDeLaCita)(servicesData, descuento, transaction);
    }
    const filas = filasDelNavegador(servicesData);
    try {
        const { filas: calculadas } = yield (0, exports.calcularFilasDeLaCita)(servicesData, descuento, transaction);
        registrarDiferencias(idCita, filas, calculadas);
    }
    catch (error) {
        console.warn(`[precio-sombra] cita ${idCita}: no se pudo calcular el precio (${error.message})`);
    }
    ...
    return { filas, precioCita: precioDelNavegador };
});
exports.resolverFilasDeLaCita = resolverFilasDeLaCita;
/*
 ...
 * está tomada acá.
 */
//# sourceMappingURL=calcularPreciosCita.js.map